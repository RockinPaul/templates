...
- **License** (must allow redistribution; all six were MIT/Apache-2.0), **release cadence** (daily releases mean frequent tag bumps), **resource needs** (Helm `values.yaml` limits are a good source).
...
3. Publish:
   ...
   ```
   `--image` must be a URL (a local path fails with "Invalid image URL"); use a raw GitHub URL of an upstream screenshot or logo. Description longer than 75 characters is rejected.
4. One-click verification: `mkdir v && cd v && railway init -n <name>-template-verify && railway deploy -t <code>`. If the template has empty required defaults the command prompts and dies without a TTY; pass `-v service.KEY=value`. Wait for all services, run §3.7 against the new domain, read generated secrets' lengths from `railway variables --json`, then `projectDelete`.
...
- Images often lack `curl`, `ps`, `timeout`; `bash` `/dev/tcp` and `/proc` are your tools.
...
- `railway deploy -t` prompts for missing defaults and fails without a TTY (`-v svc.KEY=val`).
