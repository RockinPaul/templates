...
How to turn an open-source project into a one-click Railway marketplace template, written down after building and publishing seven of them in September 2026: **cognee**, **Multica**, **projectmem**, **gortex**, **Observal**, **WeKnora** and **EverOS**. Everything here was learned the hard way on real deployments; the "gotcha" sections are the most valuable part. The guide is written for a developer or an agent who has never touched Railway templates and has to repeat this end to end.
...
- **License** (must allow redistribution; all seven were MIT/Apache-2.0), **release cadence** (daily releases mean frequent tag bumps), **resource needs** (Helm `values.yaml` limits are a good source).
