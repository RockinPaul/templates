...
- **That 50 GB is a ceiling, not a bill, and it cannot be undone.** `sizeMB` is the cap; `currentSizeMB` is real consumption, and the billed metric is `DISK_USAGE_GB` — so an over-large allocation costs nothing by itself. There is also **no way to resize an existing volume**: `VolumeUpdateInput` carries only `name`, `VolumeInstanceUpdateInput` only `mountPath`/`serviceId`/`state`, and the schema has no resize mutation. Setting the size at template-generation time is the only lever; for an existing volume the sole path down is delete and recreate, which destroys the data.
